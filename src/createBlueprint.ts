 ...
 * @param saveGame The save game is needed to look up stuff in the subsystems
...
class BlueprintCreator {
  public blueprint: SatisfactoryBlueprint;
  private origin: Vector3;
  private pathNames: { [id: string]: string } = {};
        ...
        if (inventoryStacks !== undefined) {
          ...
          for (const stack of inventoryStacks.value.values) {
            ...
            if (
              ...
              building.powerShards += item.value.properties[0].value;
               // TODO make sure to select the NumItems property
            }
          }
        }
      }
  ...
  private createConveyorInputOutput(
    ...
    for (const child of actor.entity.children) {
      ...
      // the last character of the path name of the
      // component defines the connected slot, e.g. Output3
      let connectedSlot;
      const lastChar =
  ...
  /**
   * Fetches the path name that is used in the blueprint or
   * undefined if this actor is not included in the blueprint
   */
...
function getProperty(actor: Actor, propertyName: string): Property | undefined {
  if (actor.entity !== undefined) {
    if (actor.entity.properties !== undefined) {
      for (const property of actor.entity.properties) {
...
function getPropertyFromComponent(
  ...
  if (component.entity !== undefined) {
    if (component.entity.properties !== undefined) {
      for (const property of component.entity.properties) {
        if (property.name === propertyName) {
          return property;
        }
      }
...
function findActorByName(
  ...
  for (const element of saveGame.actors) {
...
function findComponentByName(
  ...
  // TODO might be worth optimizing using hashmap or the like
  for (const element of saveGame.components) {
    if (element.pathName === pathName) {
      return element;
    }
  }
